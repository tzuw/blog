...
def rmSpecialCharacter(str):
    str = str.replace('/','-').replace('[','|').replace(']','|').replace('【','|').replace('】','|') \
        .replace('：','|').replace('+','|').replace(' ','-').replace('(','|').replace(')','|')
    str = str.replace('--','-').replace('---','-')
    return str

def parsePublishTime(time):
    return time.split('于')[1]
...
def CrawlingItemBlog(base_url, id):
    ...
    if item_html.status_code == 200:    # 200说明request_get完成，这是因为http协议里面定义的状态码
        ...
        jekyll_title = 'title:   ' + '"' + file_name.replace(':','')
...
#         jekyll_date = 'date:   ' + year + '-' + month + '-' + day + ' ' + minite + '\n'
        ...
        jekyll_date = 'date:   ' + parsePublishTime(time) + '\n'
...
#             md_file_name = './_posts/' + year + '-' + month + '-' + day + '-' + file_name + '.markdown'
        file_name = rmSpecialCharacter(file_name)
        jekyll_title = ': '.join(jekyll_title.split(':')) + '"'
        md_file_name = posts_dir + parsePublishTime(time).split(' ')[0] + '-' + file_name + '.markdown'
        print md_file_name
        f = codecs.open(md_file_name, 'w', encoding='utf-8')
        jekyll_str = '---\n' + 'layout:  post\n' + jekyll_title + "\n" + \
            jekyll_date + 'author:  "' + author + '"\nheader-img: "img/post-bg-2015.jpg"\ncatalog:   false\n' + \
...
#                 folder_path = './imgs/' + year + '-' + month + '-' + day + '-' + file_name
                folder_path = img_dir + parsePublishTime(time).split(' ')[0] + '-' + file_name
                mkdir(folder_path)
            ...
            try:
                ...
                # imghdr.what(img_name)用来判断图片真实格式
                img_real_name = folder_path + "/" + time.split('于')[2].split(' ')[0] + '-' + file_name + "_图" + str(k) +  "." + imghdr.what(img_name)
...
#     username = 'Tang_Chuanlin'        # CSDN 个人主页地址中的用户名，例如https://blog.csdn.net/Tang_Chuanlin中的Tang_Chuanlin
    username = 'qq_33387068'        # CSDN 个人主页地址中的用户名，例如https://blog.csdn.net/Tang_Chuanlin中的Tang_Chuanlin
    author = 'tzuw'
    img_dir = '../src/public/assets/img/csdn/'
    posts_dir = '../src/_drafts/csdn/'

    if os.path.exists(img_dir):      # 判断当前路径下是否存在"imgs"文件夹
        shutil.rmtree(img_dir)       # 若存在，则删除该文件夹，目的是删除之前爬取获得的图片
    mkdir(img_dir)                   # "imgs"新建文件夹
    if os.path.exists(posts_dir):      # 判断当前路径下是否存在"_posts"文件夹
        shutil.rmtree(posts_dir)       # 若存在，则删除该文件夹，目的是删除之前爬取获得的markdown文件
    mkdir(posts_dir)                   # "_posts"新建文件夹
